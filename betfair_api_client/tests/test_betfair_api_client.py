...
TEST_COMPETITION = Competition(
    competitionId=12117172, competitionName="Australian A-League"
)
TEST_EVENT = Event(
    eventId=29939806,
...
TEST_MARKET = Market(
    marketId="1.171796736",
    marketName="Match Odds",
    marketStartTime=datetime(year=2020, month=8, day=4, hour=9, minute=30, second=0),
)
FIRST_TEST_RUNNER = Runner(
    runnerId=6480414, runnerName="Western Sydney Wanderers", handicap=0.0
)
...
class TestBetfairClient(TestCase):
        ...
        self.rawMatchOddsData = [
            {
                ...
                "marketStartTime": TEST_MARKET.marketStartTime.strftime(
                    BetfairApiClient.BETFAIR_DATETIME_FORMAT
                ),
                "runners": [
                    {
                ...
                "competition": {
                    "id": str(TEST_COMPETITION.competitionId),
                    "name": TEST_COMPETITION.competitionName,
                },
                "event": {
                    "id": str(TEST_EVENT.eventId),
                    "name": TEST_EVENT.eventName,
                    "countryCode": TEST_EVENT.countryCode,
                    "openDate": TEST_EVENT.eventDate.strftime(
                        self.betfairClient.BETFAIR_DATETIME_FORMAT
                    ),
                },
            }
        ]
    ...
    def test_list_competitions(self):
        results = self.betfairClient.list_competitions(
            countryCodes=["GB"], sportTypeIds=[1]
        )
        self.assertTrue(
            Competition(
                competitionId=10932509, competitionName="English Premier League"
            )
            in results
        )

    def test_get_coming_events(self):
        comingEvents = self._get_test_events_from_betfair_api(
            betfairClient=self.betfairClient
        )
    ...
    def test_process_raw_markets_data(self):
        processedEvent = self.betfairClient._process_raw_markets_data(
            rawMarketsData=self.rawMatchOddsData
        )[0]
        ...
        self.assertEqual(
            {r.runnerId for r in processedRunners},
            {r.runnerId for r in ALL_TEST_RUNNERS},
        )
        self.assertEqual(
            {r.runnerName for r in processedRunners},
            {r.runnerName for r in ALL_TEST_RUNNERS},
        )
        self.assertEqual(
            {r.handicap for r in processedRunners},
            {r.handicap for r in ALL_TEST_RUNNERS},
        )
    ...
    def test_update_prices_for_markets_with_existing_runners(self):
        ...
        updatedMarkets = self.betfairClient.update_prices_for_markets(
    ...
    def test_update_prices_for_markets_with_no_existing_runners(self):
        ...
        self.assertRaises(
            Exception, self.betfairClient.update_prices_for_markets, [exampleMarket]
        )
